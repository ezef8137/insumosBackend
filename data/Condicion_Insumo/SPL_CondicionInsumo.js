...
const SPL_CondicionInsumo = async (req, res) => {
  const { FiltroCondicionInsumo } = req.body;

  try {
    const pool = await connect(); // Obtenemos la conexión de la función connect
    const result = await pool.request()
      .input('FiltroCondicionInsumo', FiltroCondicionInsumo)
      .execute('SPL_CondicionInsumo'); // Ejecuta el procedimiento almacenado en SQL Server
...
module.exports = { SPL_CondicionInsumo  };
